...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      try {
        ...
        const res = await axios.get("https://talentforge-backend-sbpr.onrender.com/api/notifications/unread-count",
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        const token = localStorage.getItem("token");
        const res = await axios.get(`https://talentforge-backend-sbpr.onrender.com/api/connections/search?q=${encodeURIComponent(searchQuery)}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        <aside style={{
          ...
          <div style={{ padding: "16px 20px", borderBottom: "1px solid #1e293b", display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: getRoleColor(user?.role), display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "700", fontSize: "14px", flexShrink: 0, overflow: "hidden" }}>
              {user?.profilePicture ? <img src={`https://talentforge-backend-sbpr.onrender.com${user.profilePicture}`} alt="avatar" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : getInitials(user?.fullName)}
            </div>
