...
const StudentDashboard = () => {
  ...
  const fetchSuggestions = async () => {
    try {
      const res = await axios.get("https://talentforge-backend-production.up.railway.app/api/suggestions", { headers });
  ...
  const fetchProfileViews = async () => {
    try {
      const res = await axios.get("https://talentforge-backend-production.up.railway.app/api/profile-views/my-views", { headers });
  ...
  const handleConnect = async (userId) => {
    ...
    try {
      await axios.post("https://talentforge-backend-production.up.railway.app/api/connections/request", { recipientId: userId }, { headers });
              ...
              {profileViewers.slice(0, 5).map(v => (
                <Link to={`/user/${v.viewedBy._id}`} key={v._id} style={{ display: "flex", alignItems: "center", gap: "10px", textDecoration: "none" }}>
                  <div style={{ width: "38px", height: "38px", borderRadius: "50%", background: getRoleColor(v.viewedBy.role), display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "700", fontSize: "13px", color: "white", flexShrink: 0, overflow: "hidden" }}>
                    {v.viewedBy.profilePicture ? <img src={`https://talentforge-backend-production.up.railway.app${v.viewedBy.profilePicture}`} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : getInitials(v.viewedBy.fullName)}
                  </div>
                  <div style={{ minWidth: 0 }}>
